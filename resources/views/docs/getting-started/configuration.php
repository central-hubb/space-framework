<article>
    ...
    <h2><a href="#environment-configuration">Environment Configuration</a></h2>
    ...
    <p>Your <code class=" language-php"><span class="token punctuation">.</span>env</code> file should not be committed
        to your application's source control, since each developer / server using your application could require a
        different environment configuration. Furthermore, this would be a security risk in the event an intruder gains
        access to your source control repository, since any sensitive credentials would get exposed.</p>
    <p>If you are developing with a team, you may wish to continue including a <code class=" language-php"><span
                    class="token punctuation">.</span>env<span class="token punctuation">.</span>example</code> file
        ...
        <code class=" language-php"><span class="token punctuation">.</span>env<span class="token punctuation">.</span>testing</code>
        file. This file will override the <code class=" language-php"><span class="token punctuation">.</span>env</code>
        file when running PHPUnit tests or executing Artisan commands with the <code class=" language-php"><span
                    class="token operator">--</span>env<span class="token operator">=</span>testing</code> option.</p>
    <blockquote class="has-icon">
        ...
        Any variable in your <code class=" language-php"><span class="token punctuation">.</span>env</code> file can be
        overridden by external environment variables such as server-level or system-level environment variables.</p>
    </blockquote>
    <p><a name="environment-variable-types"></a></p>
    <h3>Environment Variable Types</h3>
    <p>All variables in your <code class=" language-php"><span class="token punctuation">.</span>env</code> files are
        parsed as strings, so some reserved values have been created to allow you to return a wider range of types from
        the <code class=" language-php"><span class="token function">env<span
                        class="token punctuation">(</span></span><span class="token punctuation">)</span></code>
        function:</p>
    <table>
        <thead>
        <tr>
            <th><code class=" language-php"><span class="token punctuation">.</span>env</code> Value</th>
            <th><code class=" language-php"><span class="token function">env<span
                                class="token punctuation">(</span></span><span class="token punctuation">)</span></code>
                Value
            </th>
        </tr>
