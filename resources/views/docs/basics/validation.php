...
<span class="token punctuation">]</span><span class="token punctuation">)</span><span class="token punctuation">;</span></code></pre>
	...
	<p>In this example, we used a traditional form to send data to the application. However, many applications use AJAX requests. When using the <code class=" language-php">validate</code> method during an AJAX request, Space MVC will not generate a redirect response. Instead, Space MVC generates a JSON response containing all of the validation errors. This JSON response will be sent with a 422 HTTP status code.</p>
...
<span class="token punctuation">}</span></code></pre>
	<p>Since all form requests extend the base Space MVC request class, we may use the <code class=" language-php">user</code> method to access the currently authenticated user. Also note the call to the <code class=" language-php">route</code> method in the example above. This method grants you access to the URI parameters defined on the route being called, such as the <code class=" language-php"><span class="token punctuation">{</span>comment<span class="token punctuation">}</span></code> parameter in the example below:</p>
...
<span class="token punctuation">]</span><span class="token punctuation">,</span></code></pre>
	...
	<p>Space MVC provides a variety of helpful validation rules; however, you may wish to specify some of your own. One method of registering custom validation rules is using rule objects. To generate a new rule object, you may use the <code class=" language-php">make<span class="token punctuation">:</span>rule</code> Artisan command. Let's use this command to generate a rule that verifies a string is uppercase. Space MVC will place the new rule in the <code class=" language-php">app<span class="token operator">/</span>Rules</code> directory:</p>
