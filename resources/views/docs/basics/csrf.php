...
<article>
    ...
    <p>Space MVC makes it easy to protect your application from <a href="https://en.wikipedia.org/wiki/Cross-site_request_forgery">cross-site request forgery</a> (CSRF) attacks. Cross-site request forgeries are a type of malicious exploit whereby unauthorized commands are performed on behalf of an authenticated user.</p>
    <p>Space MVC automatically generates a CSRF "token" for each active user session managed by the application. This token is used to verify that the authenticated user is the one actually making the requests to the application.</p>
    <p>Anytime you define a HTML form in your application, you should include a hidden CSRF token field in the form so that the CSRF protection middleware can validate the request. You may use the @csrf Blade directive to generate the token field:</p>
    <?php echo Code::getHtmlStatic('&lt;form method="POST" action="/profile"&gt;
    @csrf
    ...
&lt;/form&gt;'); ?>
    <p>The VerifyCsrfToken <a href="/docs/5.7/middleware">middleware</a>, which is included in the web middleware group, will automatically verify that the token in the request input matches the token stored in the session.</p>
...
class VerifyCsrfToken extends Middleware
{
    ...
    <p>The CSRF middleware is automatically disabled when <a href="/docs/5.7/testing">running tests</a>.</p>
    <p><a name="csrf-x-csrf-token"></a></p>
    <h2><a href="#csrf-x-csrf-token">X-CSRF-TOKEN</a></h2>
    <p>In addition to checking for the CSRF token as a POST parameter, the VerifyCsrfToken middleware will also check for the X-CSRF-TOKEN request header. You could, for example, store the token in a HTML meta tag:</p>
    ...
    <?php echo Code::getHtmlStatic('$.ajaxSetup({
    headers: {
    ...
    <p>Space MVC stores the current CSRF token in a XSRF-TOKEN cookie that is included with each response generated by the framework. You can use the cookie value to set the X-XSRF-TOKEN request header.</p>
